...
class RMA_full(TorchModelV2, nn.Module):
    def __init__(
        ...
        if self.train_adaptation:  # if we are training the adaptation module, we need a history of actions
            self.view_requirements = {
                "obs": ViewRequirement(shift="-{}:0".format(self.seq_len - 1), space=self.obs_space, batch_repeat_value=self.seq_len),
                "prev_actions": ViewRequirement(shift="-{}:-1".format(self.seq_len), data_col='actions', space=self.action_space, batch_repeat_value=self.seq_len)
            }
        else:
            self.view_requirements = {
        ...
        self.adaptation_module = TimeCNN(self.num_states + self.num_actions, self.param_embed_dim, self.seq_len)
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        if self.train_adaptation:
            s_a_history = torch.cat((obs[:, :, :self.num_states], prev_actions), dim=-1)  # concatenate state and action histories
            self.z_hat = self.adaptation_module(s_a_history)
            with torch.no_grad():
                e = obs[:, 0, self.num_states:self.num_states + self.num_params]  # flattened drone parameters
                self.z = self.param_encoder(e)  # encode drone parameters
                self._features = self._hidden_layers(torch.cat((s_a_history[:, -1], self.z), dim=-1))
            ...
            e = obs[:, self.num_states:self.num_states + self.num_params]  # flattened drone parameters
            self.z = self.param_encoder(e)  # encode drone parameters
...
class TimeCNN(nn.Module):
    def __init__(self, in_feature_dim, param_embed_dim, seq_len=50):
        ...
        self.outMLP = nn.Sequential(
            ...
            SlimFC(in_size=32, out_size=param_embed_dim, initializer=nn.init.xavier_normal_, activation_fn='tanh')
        )
