...
class CustomLSTM(RecurrentNetwork, nn.Module):
    ...
    def __init__(
        ...
        self.MLP1 = nn.Sequential(
            SlimFC(in_size=input_size, out_size=64, initializer=nn.init.xavier_normal_, activation_fn='tanh'),
        )
        ...
        self._value_branch = nn.Sequential(
            SlimFC(in_size=64, out_size=128, initializer=nn.init.xavier_normal_, activation_fn='tanh'),
...
class CustomLSTMbigger(RecurrentNetwork, nn.Module):
...
class CustomLSTMbiggerCommonF(RecurrentNetwork, nn.Module):
    ...
    def forward_rnn(self, inputs, state, seq_lens, is_training):
        ...
        f = self.MLP1(inputs)
        f = self.bn(f.transpose(1, 2)).transpose(1, 2)  # apply batch norm
        self._features, [h, c] = self.LSTM(f, [torch.unsqueeze(state[0], 0), torch.unsqueeze(state[1], 0)])
        self._features = f + self._features
