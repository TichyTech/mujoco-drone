...
def make_drone(id=0, hue=1):
    ...
    for i in range(4):
        ...
        model.worldbody.add('geom', name='arm_%d' % i, size=(0.05, 0.005, 0.005), pos=hip_pos, euler=[0, 0, theta], rgba=rgba, mass=0.025)
        model.worldbody.add('site', name='motorsite_%d' % i, type='cylinder', pos=1.6*hip_pos + np.array([0, 0, 0.0075]), size=(0.01, 0.0025), rgba=rgba)
        model.worldbody.add('geom', name='prop_%d' % i, type='cylinder', pos=1.6 * hip_pos + np.array([0, 0, 0.01]), size=(0.05, 0.0025), rgba=rgba, mass=0.025)
        model.actuator.add('motor', name='motor_%d' % i, site='motorsite_%d' % i, gear=(0, 0, 1, 0, 0, 0.1*(-1)**i), ctrllimited=True, ctrlrange=(0, 1))
    return model
